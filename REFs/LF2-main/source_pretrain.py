...
from reid.trainers import PreTrainer
...
def main_worker(args):
    ...
    if args.evaluate:
        ...
        evaluator.evaluate(test_loader_source, dataset_source.query, dataset_source.gallery, cmc_flag=True, rerank=args.rerank, source=True)
        print("Test on target domain:")
        evaluator.evaluate(test_loader_target, dataset_target.query, dataset_target.gallery, cmc_flag=True, rerank=args.rerank, source=True)
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            ...
            _, mAP = evaluator.evaluate(test_loader_source, dataset_source.query, dataset_source.gallery, cmc_flag=True, source=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dt', '--dataset-target', type=str, default='market',
                        choices=datasets.names())
    ...
    parser.add_argument('--num-instances', type=int, default=4,
                        help="each minibatch consist of "
                             ...
                             "default: 0 (NOT USE)")
    # model
    parser.add_argument('-a', '--arch', type=str, default='resnet50_source',
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(os.getcwd(), 'logs_duke_pretrained_multi_gpu'))
    main()
