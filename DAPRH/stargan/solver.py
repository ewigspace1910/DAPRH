...
class Solver(object):
    ...
    def __init__(self, celeba_loader, rafd_loader, config):
        ...
        self.except_domain = config.except_domain
        self.pattern       = config.pattern
        self.prob          = 1 - max(min(config.prob, 1.), 0)
    ...
    def samples(self):
        ...
        with torch.no_grad():
            for i in range(lenDS):
                x_fixed, pid = next(data_iter)
                assert len(pid) == 1, "only using batch size 1 for this mode"
                x_fixed = x_fixed.to(self.device)
                c_fixed_list = self.create_labels(pid, self.c_dim, self.dataset, self.selected_attrs)
                x_fake_list = []
                for ii, c_fixed in enumerate(c_fixed_list):
                    ...
                    str_id  = "0"*(4-len(str(pid[0].item()))) + str(pid[0].item())
                    str_cam = "c" + str(torch.argmax(c_fixed).item())
                    str_ran = str(i*25)
                    if self.pattern.find("RANDOM") :save_name = self.pattern.format(ID=str_id, CX=str_cam, RANDOM=str_ran)
                    else: save_name = self.pattern.format(ID=str_id, CX=str_cam)
                    sample_path = os.path.join(self.gen_dir, save_name)
                    save_image(self.denorm(x_concat.data.cpu()), sample_path, nrow=1, padding=0)
                    print('Saved  fake images into {}'.format(sample_path))
                print("---1 real -> {} fake ---".format(len(x_fake_list)))