...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, num_features=0, norm=False, dropout=0, num_classes=0, **kwargs):
        super(ResNet, self).__init__()

        self.pretrained = pretrained
        self.depth = depth
        # Construct base (pretrained) resnet
        if depth not in ResNet.__factory: raise KeyError("Unsupported depth:", depth)
        if depth >= 50: resnet = ResNet.__factory[depth](pretrained="IMAGENET1K_V2") #(pretrained=pretrained)
        else: resnet = ResNet.__factory[depth](pretrained=pretrained)
        if depth >= 50:
    ...
    def forward(self, x, feature_withbn=False, joint=False):
        ...
        if self.num_classes > 0: 
            ...
            if feature_withbn: return bn_x, prob
            return x, prob
        else: return x, bn_x
