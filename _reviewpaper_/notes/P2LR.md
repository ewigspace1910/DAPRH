...
Through probabilistic uncertainty in III.#43, we will choose all samples ($x_{i}$) having $U$ \'s value is smaller than a threshold $\beta$, which a muable scalar. $\beta$ can update(increase) over interators.
