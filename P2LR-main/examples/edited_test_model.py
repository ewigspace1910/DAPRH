...
def main_worker(args):
    ...
    copy_state_dict(checkpoint['state_dict'], model1)
    start_epoch = checkpoint['epoch']
    best_mAP = checkpoint['best_mAP']
    print("=> Checkpoint 1 of epoch {}  best mAP {:.1%}".format(start_epoch, best_mAP))
    
    checkpoint = load_checkpoint(args.model2)
    copy_state_dict(checkpoint['state_dict'], model2)
    start_epoch = checkpoint['epoch']
    best_mAP = checkpoint['best_mAP']
    print("=> Checkpoint 2 of epoch {}  best mAP {:.1%}".format(start_epoch, best_mAP))

    # Evaluator
    evaluator = Evaluator(model1, model2, args)
    ...
    evaluator.evaluate(test_loader_target, dataset_target.query, dataset_target.gallery, 
            cmc_flag=False, rerank=args.rerank, use_kmean=args.kmeans)
    return

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Testing the model with unlabel dataset, matches all images in gallery that most clost with probe")
    # data
